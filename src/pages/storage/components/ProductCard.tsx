    ...
    >
      <div className="flex w-full lg:hidden overflow-x-auto bg-accent overflow-y-hidden touch-pan-y snap-x snap-mandatory [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        <div
          ...
          style={{
            borderLeft: `36px solid ${getStatusColor(item.quantity)}`,
          }}
          onClick={() => toggleCard(item.id)}
        >
          <div className="flex flex-col justify-center py-4 pl-6 min-h-22">
            <span className={`font-bold font-poppins text-primary-text w-full text-md`}>
              {item.volume == 0 ? `${item.product_name} (un)` : `${item.product_name} (${item.volume}${item.measure})`}
            </span>
          ...
          <div
            className={`grid transition-all duration-300 ease-in-out ${expandedCardId === item.id
        ...
        <div className="flex gap-2 w-28 min-w-28 bg-white overflow-x-auto scroll-smooth snap-x snap-mandatory">
          <div className="flex items-center justify-center gap-2 min-w-28 snap-end bg-white">
            <img
              src={addItemIcon}
          ...
          <div
            className="flex-none flex items-center justify-center pl-4 w-28 min-w-28 snap-start cursor-pointer text-white font-poppins font-bold text-lg active:opacity-80 transition-opacity"
      ...
      >
        <div className="flex items-start justify-between">
          <div
            ...
            style={{
              backgroundColor: `color-mix(in srgb, ${getStatusColor(item.quantity)}, transparent 75%)`,
          ...
          <div className="flex items-center gap-2">
            ...
            <span
              ...
              style={{
                backgroundColor: `${getStatusColor(item.quantity)}22`,
                color: getStatusColor(item.quantity),
              }}
            >
        ...
        <span className="font-bold text-[15px] text-primary-text leading-tight font-poppins">
          {item.volume == 0 ? item.product_name : `${item.product_name} (${item.volume}${item.measure})`}
        </span>
