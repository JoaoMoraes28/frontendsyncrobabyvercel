...
function ProfileChildren() {
    ...
    const [genderSelected, setGenderSelected] = useState<string>(dataChildren.gender)
    ...
    return (
        <div className="flex w-screen h-screen pt-24 pb-22
        md:pb-28
        xl:pt-0 xl:pb-0" >
            <div className="xl:">
                <Header />
            </div>
            <Perfil register_name={register("name")} genderSelected={genderSelected} setGenderSelected={setGenderSelected} readonly={onlyRead} child={dataChildren} />
            <div className="flex w-full h-full
            xl:justify-center xl:items-center xl:pt-10 bg-light">
                <form onSubmit={handleSubmit(sendDatas)} className="flex flex-col space-y-[10%] w-full h-full pb-2 overflow-y-auto px-6
                md:px-14
                xl:bg-lilas xl:w-[85%] xl:h-[80%] xl:max-w-340 xl:max-h-250 xl:py-6 xl:space-y-0 xl:rounded-2xl xl:shadow-purple-md">
                    <h2 className="hidden xl:block xl:-ml-7 xl:font-poppins xl:text-darker-purple xl:font-extrabold xl:text-[3.3rem]">Seus Dados</h2>
                    <div className="w-full
                    xl:hidden">
                        <div className="flex justify-between items-start">
                            <img src={exportIcon} alt="Gera um arquivo PDF com os dados da criança." className="mt-2 w-auto h-5" />
                            <img src={ProfilePicture} alt="Foto de perfil da criança." />
                            <div className="relative flex w-4 h-6">
                                <button onClick={cancelChanges} type="button" className={`absolute top-2 right-9 w-4 h-4 scale-140 ${onlyRead ? 'hidden' : 'block'}`}>
                                    <img src={Cancel} alt="Cancela as alterações no modo editar." className="w-full h-full" />
                                </button>
                                <button onClick={() => setOnlyRead(!onlyRead)} type={onlyRead ? 'submit' : 'button'} className="absolute" >
                                    <img src={onlyRead ? EditIcon : Confirm} alt="Habilita a edição dos dados da criança." className="mt-2 w-auto h-4" />
                                </button>
                            </div>
                        </div>
                        <h3 className="flex justify-center items-center font-poppins text-primary-text font-bold text-3xl w-full h-20">
                            <InputDefault readOnly={onlyRead} {...register("name")} className={`w-[75%] text-center ${onlyRead ? '' : 'border-2 border-primary rounded-lg'}`} />
                        </h3>
                        <section className="flex items-center w-full h-20 font-poppins font-semibold text-primary-text border rounded-lg bg-white border-primary shadow-purple-sm">
                            <div className="flex justify-center items-center gap-3 w-1/3 h-[70%]">
                                <button onClick={() => setGenderSelected('masculino')} type="button" className={`flex justify-center items-center w-12 h-12 rounded-lg ${genderSelected == 'masculino' && !onlyRead ? 'bg-lilas/80 border border-primary' : ''} ${dataChildren.gender == 'masculino' || !onlyRead ? 'flex' : 'hidden'}`}>
                                    <img src={Male} alt="Gênero masculino." className="w-auto h-7" />
                                </button>
                                <button onClick={() => setGenderSelected('feminino')} type='button' className={`flex justify-center items-center w-12 h-12 rounded-lg ${genderSelected == 'feminino' && !onlyRead  ? 'bg-lilas/80 border border-primary' : ''} ${dataChildren.gender == 'feminino' || !onlyRead ? 'flex' : 'hidden'}`}>
                            ...
                            <span className="flex justify-center items-center w-1/3 h-[70%] text-lg">IMC: {dataChildren.imc}</span>
                        </section>
                    </div>
                    <dl className="flex flex-col justify-center gap-2 w-full grow">
                        {descriptionItems.map((it) => (
                            <AtributesProfile key={it.title} date_birth={register("date_birth")} blood_type={register("blood_type")} listDescription={it} onlyRead={onlyRead} />
                        ))}
                    </dl>
                    <div className="hidden xl:flex xl:justify-between xl:w-full">
                        <BtnPrimary onClick={cancelChanges} type="button" text="Cancelar" className="xl:bg-white xl:w-2/5 xl:max-w-90 xl:h-12 xl:font-semibold xl:-ml-7 xl:shadow-purple-md" />
                        <BtnPrimary onClick={() => setOnlyRead(!onlyRead)} type={onlyRead ? 'submit' : 'button'} text={onlyRead ? 'Editar' : 'Salvar'} className={`xl:w-1/3 xl:max-w-76 xl:h-12 xl:font-semibold xl:text-white xl:shadow-purple-md ${onlyRead ? 'xl:bg-primary' : 'xl:bg-primary-darker'}`} />
                    </div>
                </form>
            </div>
