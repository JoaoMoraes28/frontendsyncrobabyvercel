...
export const getProductsByType = async (childId: number, type_id: number): Promise<ResponseGetStorage> => {
    try {
        const response = await api.get<ResponseGetStorage>(`https://syncrobabybackend-hmc2g7cqe9bfbqcr.brazilsouth-01.azurewebsites.net/syncrobaby/stock/type?child=${childId}&type=${type_id}`);
        return response.data;
    } catch (error: any) {
        if (String(error).includes("404")) {
            return {
